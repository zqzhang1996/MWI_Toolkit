...
(function (): void {
    ...
    class RequiredItem extends DisplayItem {
        ...
        displayNameSpan: HTMLSpanElement = null;
        ...
        updateDisplayElement() {
            ...
            if (this.displayNameSpan) {
                ...
                if (netCount < 0) { this.displayNameSpan.style.color = '#eb3f3f'; }
                else if (netCount < this.count) { this.displayNameSpan.style.color = '#faa21e'; }
                else if (netCount < 2 * this.count) { this.displayNameSpan.style.color = ''; }
                else if (netCount < 4 * this.count) { this.displayNameSpan.style.color = '#59d0b9'; }
                else { this.displayNameSpan.style.color = '#4aa3e6'; }
            }
        }
        removeDisplayElement() {
    ...
    class MWI_Toolkit_Calculator {
        ...
        static saveDetailsOpenState(): void {
            ...
            const detailsState: Record<string, boolean> = {};
        ...
        static restoreDetailsOpenState(storageKey_Details: string): void {
            try {
                const saved = GM_getValue(storageKey_Details, '{}');
                const detailsState: Record<string, boolean> = JSON.parse(saved);
        ...
        // 保存数据
        static saveCalculatorData(): boolean {
            const storageKey = MWI_Toolkit_Calculator.getStorageKey();
        ...
        // 从特定角色ID加载数据
        static loadCalculatorData(characterID: number | null = null) {
                ...
                if (processedItemHrid) {
                    ...
                    if (processedItemOutputCount !== 0 && processedItemShortageCount !== 0) {
                        ...
                        if (requiredItem.shortageCount < (processedItemShortageCount / processedItemOutputCount * outputCount)) {
                            ...
                            const actionCount = MWI_Toolkit_Calculator.getRequiredTrials99(outputCount,
                                requiredItem.shortageCount / (outputCount + processedItemOutputCount * processedItemInputCount) * outputCount);
                            MWI_Toolkit.gameObject.handleGoToAction(actionHrid, actionCount);
                        } else {
                            // 加工茶效果产物可以完全覆盖需求，从原材料中扣除总数对应原料
                            const actionCount = MWI_Toolkit_Calculator.getRequiredTrials99(outputCount,
                                requiredItem.shortageCount - processedItemShortageCount * processedItemInputCount);
                            MWI_Toolkit.gameObject.handleGoToAction(actionHrid, actionCount);
                        }
                        return;
                    }
    ...
    class UpgradeItemComponent {
        itemHrid: string;
        count: number;
        shortageCountSpan: HTMLSpanElement;
    }
    ...
    class InputItemComponent {
        ...
        count: number;
        shortageCountSpan: HTMLSpanElement;
        inventoryCountSpan: HTMLSpanElement;
        inputCountSpan: HTMLSpanElement;
    }

    class OutputItemComponent {
        count: number;
        outputItemInput: HTMLInputElement;
    }
    ...
    class MWI_Toolkit_ActionDetailPlus {
        ...
        static processableActionMap: Map<string, string> = new Map([
        ...
        static processableItemMap: Map<string, string> = new Map([
        ...
        static upgradeItemComponent: UpgradeItemComponent;
        static inputItemComponents: Array<InputItemComponent>;
        static outputItemComponents: Array<OutputItemComponent>;
        static processingTeaComponent: ProcessingTeaComponent;
        ...
        static enhanceSkillActionDetail() {
            ...
            function updateSkillActionDetail(e: Event) {
                ...
                if (MWI_Toolkit_ActionDetailPlus.upgradeItemComponent) {
                    if (isNaN(skillActionTimes)) { MWI_Toolkit_ActionDetailPlus.upgradeItemComponent.shortageCountSpan.textContent = ''; }
                    else {
        ...
        // 创建升级物品组件
        static createUpgradeItemComponent(upgradeItemHrid: string) {
            MWI_Toolkit_ActionDetailPlus.upgradeItemComponent = null;

            if (!upgradeItemHrid) { return; }
            ...
            if (!shortageCountContainer) { return; }
        ...
        // 创建所有输入物品组件
        static createInputItemComponents(inputItems: Array<ItemWithCount>) {
            MWI_Toolkit_ActionDetailPlus.inputItemComponents = new Array<InputItemComponent>();
        ...
        // 创建所有输出物品组件
        static createOutputItemComponents(outputItems: Array<ItemWithCount>) {
            MWI_Toolkit_ActionDetailPlus.outputItemComponents = Array<OutputItemComponent>();
        ...
        // 创建输出物品组件
        static createOutputItemComponent(outputItem: ItemWithCount): HTMLDivElement {
        ...
        // 创建加工茶产出组件
        static createProcessingTeaComponent(processedItem: ItemWithCount, outputItems: Array<ItemWithCount>): HTMLDivElement {
            const origComponent = document.querySelector('[class^="SkillActionDetail_maxActionCountInput"]') as HTMLDivElement;
            if (!origComponent) return null;
            // 克隆外层div（不带子内容）
            const component = origComponent.cloneNode(false) as HTMLDivElement;
            ...
            const totalInputWrap = origInputWrap.cloneNode(true) as HTMLDivElement;
        ...
        // 计算动作详情
        static calculateActionDetail(actionHrid: string): { upgradeItemHrid: string | null; inputItems: Array<ItemWithCount>; outputItems: Array<ItemWithCount> } {
    ...
    class MWI_Toolkit_Utils {
        ...
        // 创建图标SVG元素
        static createIconSvg(iconHref: string): SVGSVGElement {
